...
function sharesFor(expense) {
  ...
  if (expense.splitMode === 'self') shares[expense.payer] = Math.round(expense.twdAmount);
  else if (expense.splitMode === 'custom') state.members.forEach(m => shares[m] = Math.round(expense.twdAmount * (expense.customRatios[m] || 0) / 100));
  else {
...
function renderAll() {
  ...
  $('expenseList').innerHTML = state.expenses.length ? state.expenses.slice().reverse().map(e => { const cat = CATEGORIES.find(c => c.name === e.category); const splitLabel = e.splitMode === 'equal' ? '全體均分' : e.splitMode === 'custom' ? '自訂比例' : '自己負擔'; return `<article class="expense"><span class="category-icon">${cat.icon}</span><div><h3>${escapeHtml(e.item)}</h3><p>${e.date} · ${e.category} · ${splitLabel} · 付款人 ${e.payer}</p></div><div class="expense-amount"><span>${e.currency} ${e.originalAmount.toLocaleString()}</span><b>${money(e.twdAmount)}</b></div><button class="delete" data-id="${e.id}" aria-label="刪除">×</button></article>`; }).join('') : '<div class="empty">尚未加入消費紀錄<br/><small>從左側開始記錄這趟旅程吧！</small></div>';
...
function renderSettlement() {
  ...
  $('memberAnalytics').innerHTML = state.members.map(m => {
    const items = state.expenses.filter(e => e.payer === m);
    const paidTotal = items.reduce((sum, e) => sum + e.twdAmount, 0);
    const categories = CATEGORIES.map(c => ({ ...c, amount: items.filter(e => e.category === c.name).reduce((sum, e) => sum + e.twdAmount, 0) })).filter(c => c.amount > 0).sort((a, b) => b.amount - a.amount);
    const categoryRows = categories.length ? categories.map(c => { const pct = Math.round(c.amount / paidTotal * 1000) / 10; return `<div class="member-category"><div class="category-meta"><span>${c.icon} ${c.name}</span><span>${money(c.amount)} · <b>${pct}%</b></span></div><div class="bar"><i style="width:${pct}%;background:${c.color}"></i></div></div>`; }).join('') : '<span class="muted">尚無付款紀錄</span>';
    return `<article class="member-analytic"><div class="member-analytic-head"><b>${m}</b><span class="member-avatar">${escapeHtml(m).slice(0,1)}</span></div><div class="member-total">個人付款總額<b>${money(paidTotal)}</b></div>${categoryRows}</article>`;
  }).join('');
}
